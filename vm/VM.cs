...
public class VM
{
    ...
    {
        return lineToIP.TryGetValue((fileName, line), out ip) ? ip : -1;
    }

    public (string, int) GetLineFromIP(int ip)
    {
        return ipToLine.TryGetValue(ip, out (string, int) entry) ? entry : (null, -1);
    }
