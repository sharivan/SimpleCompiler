...
public partial class Compiler
{
    ...
    private void ParseParamsDeclaration(Function function)
    {
        while (true)
        {
            ...
            Parameter p = function.DeclareParameter(name, type, id.Interval, byRef) ?? throw new CompilerException(id.Interval, $"Parâmetro '{name}' já declarado.");
    ...
    private void ParseFieldsDeclaration(StructType st)
    {
        while (true)
        {
            ...
            Field field = st.DeclareField(name, type, id.Interval) ?? throw new CompilerException(id.Interval, $"Campo '{name}' já declarado.");
    ...
    private BlockStatement ParseBlock()
    {
        ...
        result.Interval = SourceInterval.Merge(result.Interval, lexer.CurrentInterval());
        return result;
    }
    ...
    private void ParseFunctionDeclaration()
    {
        ...
        f.Interval = SourceInterval.Merge(f.Interval, lexer.CurrentInterval());
    }
        ...
        if (kw != null)
        {
            switch (kw.Value)
            {
                ...
                case "var":
                {
                    ...
                    Variable var = unity.DeclareGlobalVariable(name, declaration.Type, declaration.Interval) ?? throw new CompilerException(declaration.Interval, $"Variável global '{name}' já declarada.");
